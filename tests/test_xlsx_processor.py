...
def test_xlsx_metadata(path, sheet_names, active_sheet, data, last_modified_by, creator):
    ...
    assert sum(len(file_obj.metadata["data"][x])
               for x in file_obj.metadata["data"].keys()) == data
...
def test_save_xlsx_metadata(copy_file, sheet_names, active_sheet, data):
    ...
    xlsx_file.save()
    test_xlsx_metadata(copy_file, sheet_names, active_sheet,
                       data, 'Modified Creator', 'New Creator')
