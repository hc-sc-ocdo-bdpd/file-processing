...
variable_names = "path, package_name, version, python_compatibility, platform_compatibility, optional_dependencies, non_optional_dependencies, build_tag, author"
values = [
    (test_files_path / 'numpy-2.1.2-pp310-pypy310_pp73-win_amd64.whl', "numpy", "2.1.2", ">=3.10", 
    ...
    (test_files_path / 'pandas-2.2.3-cp313-cp313t-musllinux_1_2_x86_64.whl', "pandas", "2.2.3", ">=3.7", "manylinux1_x86_64", "Another Author")
]
...
def test_whl_extraction(file_obj):
    # Ensure that extraction does not raise any errors
    file_obj.processor.process()

    # Check that metadata fields are correctly extracted
    metadata = file_obj.metadata
    assert metadata["package_name"] is not None
    assert metadata["version"] is not None
    assert metadata["python_compatibility"] is not None
    assert metadata["non_optional_dependencies"] is not None
    assert metadata["optional_dependencies"] is not None
    assert metadata["build_tag"] is not None
    assert metadata["author"] is not None
    assert metadata["platform_compatibility"] is not None
...
def test_whl_metadata(path, package_name, version, python_compatibility, platform_compatibility, non_optional_dependencies, optional_dependencies, build_tag, author):
    file_obj = File(path)
    metadata = file_obj.metadata
...
@pytest.mark.parametrize(variable_names, values)
def test_save_whl_metadata(copy_file, package_name, version, python_compatibility, platform_compatibility, non_optional_dependencies, optional_dependencies, build_tag, author):
    test_whl_metadata(copy_file, package_name, version, python_compatibility, platform_compatibility, non_optional_dependencies, optional_dependencies, build_tag, author)
